...
final class ResetStaleCashierSessionsCommandTest extends TestCase
{
    ...
    public function test_resets_stale_cashier_sessions_and_preserves_other_bots(): void
    {
        ...
        $staleCashierShift = $this->makeSession('cashier-shift', 'shift_count_uzs', 999_001, $user->id, ['shift_id' => 1], days: 14);
        $staleCashierPay   = $this->makeSession('cashier-pay',   'payment_fx_amount', 999_002, $user->id, ['fx_presentation' => []], days: 10);

        // Stale housekeeping & kitchen sessions — must remain untouched
        $staleHk      = $this->makeSession('hk', 'hk_main',      999_010, $user->id, ['x' => 1], days: 30);
        $staleKitchen = $this->makeSession('kitchen', 'kitchen_main', 999_011, $user->id, ['y' => 2], days: 30);

        // Recent cashier session (within 7d) — must remain untouched
        $recentCashier = $this->makeSession('recent-cashier', 'expense_amount', 999_020, $user->id, ['recent' => true], days: 1);

        // Cashier session in main_menu / idle — must remain untouched (already clean)
        $cleanCashier = $this->makeSession('clean-cashier', 'main_menu', 999_021, $user->id, null, days: 30);
        $idleCashier  = $this->makeSession('idle-cashier',  'idle',      999_022, $user->id, null, days: 30);
    ...
    {
        $user = User::factory()->create();
        $stale = $this->makeSession('dry-run', 'shift_count_uzs', 999_100, $user->id, ['shift_id' => 1], days: 30);
    ...
    private function makeSession(string $tag, string $state, int $chatId, int $userId, ?array $data, int $days): TelegramPosSession
    {
