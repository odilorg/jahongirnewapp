...
## Addendum A: Exact Legacy Config/Env Mapping Per Bot

### Bot 1: `main` (Main / Booking)

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `TELEGRAM_BOT_TOKEN` | `services.telegram.bot_token` | `TelegramBotService::__construct` (line 16) |
| `TELEGRAM_BOT_TOKEN` | `services.telegram_bot.token` | `TelegramController::__construct` (line 25), `SendTelegramMessageJob::handle` (line 36) |
| `TELEGRAM_BOT_TOKEN` | `services.telegram_booking_bot.token` | `BookingWebhookController` (constructor) |
| `TELEGRAM_BOOKING_WEBHOOK_URL` | `services.telegram_booking_bot.webhook_url` | Not read at runtime — config only |
| `TELEGRAM_BOOKING_SECRET_TOKEN` | `services.telegram_booking_bot.secret_token` | Not read at runtime — config only |
| `TELEGRAM_BOOKING_SESSION_TIMEOUT` | `services.telegram_booking_bot.session_timeout` | `TelegramBookingService` (lines 48, 208), `TelegramBookingSession` model (line 57) |

**Note:** All 3 config paths read the same `TELEGRAM_BOT_TOKEN` env var. After migration all 3 resolve to slug `main`.

### Bot 2: `driver_guide`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `TELEGRAM_BOT_TOKEN_DRIVER_GUIDE` | `services.telegram_bot.bot_token` | Not read at runtime — confusing legacy path |
| `TELEGRAM_BOT_TOKEN_DRIVER_GUIDE` | `services.driver_guide_bot.token` | `TelegramDriverGuideSignUpController::__construct` (line 23), `TourSendReminders::__construct` (line 25), `BookingObserver::__construct` (line 22), `GygNotifier::notifyOwner` (line 120) |
| `DRIVER_GUIDE_WEBHOOK_SECRET` | `services.driver_guide_bot.webhook_secret` | `TelegramDriverGuideSignUpController::__construct` (line 24) — stored but **never validated** |
| `TELEGRAM_OWNER_CHAT_ID` | `services.driver_guide_bot.owner_chat_id` | `TelegramDriverGuideSignUpController::__construct` (line 25) — default `38738713` |

### Bot 3: `pos`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `TELEGRAM_POS_BOT_TOKEN` | `services.telegram_pos_bot.token` | `TelegramPosController::__construct` (line 32), `ValidateTelegramRequest::handle` (line 22), `SetTelegramPosWebhook::handle` (line 29) |
| `TELEGRAM_POS_WEBHOOK_URL` | `services.telegram_pos_bot.webhook_url` | `SetTelegramPosWebhook::handle` (line 36) |
| `TELEGRAM_POS_SECRET_TOKEN` | `services.telegram_pos_bot.secret_token` | **Commented out** in `ValidateTelegramRequest` (line 39) |
| `TELEGRAM_POS_SESSION_TIMEOUT` | `services.telegram_pos_bot.session_timeout` | `TelegramPosController::__construct` (line 33), `TelegramPosService` (lines 63, 222), `TelegramPosSession` model (line 57) |

### Bot 4: `owner_alert`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `OWNER_ALERT_BOT_TOKEN` | `services.owner_alert_bot.token` | `OwnerAlertService::__construct` (line 23), `OwnerBotController::__construct` (line 19), `CashierBotController::__construct` (line 43 — **fallback**), `CashierBotController::handleExpensePhoto` (line 693), `QueueHealthCheck::handle` (line 54), `TourSendReminders::__construct` (line 23), `RefreshBeds24Token::handle` (line 54), `Beds24BookingService::alertOwner` (line 219) |
| `OWNER_TELEGRAM_ID` | `services.owner_alert_bot.owner_chat_id` | `OwnerAlertService::__construct` (line 24), `OwnerBotController::sendWelcomeMessage` (line 233), `CashierBotController::handleExpensePhoto` (line 692), `QueueHealthCheck::handle` (line 55), `TourSendReminders::__construct` (line 24), `RefreshBeds24Token::handle` (line 55), `Beds24BookingService::alertOwner` (line 220) |

### Bot 5: `cashier`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `CASHIER_BOT_TOKEN` | `services.cashier_bot.token` | `CashierBotController::__construct` (line 43), `OwnerBotController::approveExpense` (line 192), `AssertProductionConfig::handle` (line 51) |
| `CASHIER_BOT_WEBHOOK_SECRET` | `services.cashier_bot.webhook_secret` | `VerifyTelegramWebhook::handle` (line 30) |

**Note:** `CashierBotController` line 43 falls back to `config('services.owner_alert_bot.token')` if `CASHIER_BOT_TOKEN` is empty.
**Note:** `OwnerBotController` line 192 uses the misspelled env key `TELEGRAM_CASHIER_BOT_TOKEN` (vs correct `CASHIER_BOT_TOKEN`).

### Bot 6: `housekeeping`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `HOUSEKEEPING_BOT_TOKEN` | `services.housekeeping_bot.token` | `HousekeepingBotController::__construct` (line 30), `Beds24WebhookController::notifyHousekeeping` (line 604) |
| `HOUSEKEEPING_MGMT_GROUP_ID` | `services.housekeeping_bot.mgmt_group_id` | `HousekeepingBotController::__construct` (line 31), `Beds24WebhookController::notifyHousekeepingGroup` (line 656) |

### Bot 7: `kitchen`

| Env Var | Config Path | Consumers |
|---------|-------------|-----------|
| `KITCHEN_BOT_TOKEN` | `services.kitchen_bot.token` | `KitchenBotController::__construct` (line 23) |

---

## Addendum B: Runtime `env()` Calls Outside Config Files

These break under `php artisan config:cache` because `env()` returns `null` after config is cached.

| File | Line(s) | env() Call | Used As Fallback For |
|------|---------|-----------|---------------------|
| `app/Services/TelegramBotService.php` | 16 | `env("TELEGRAM_BOT_TOKEN")` | `config("services.telegram.bot_token", ...)` |
| `app/Services/OwnerAlertService.php` | 23 | `env('OWNER_ALERT_BOT_TOKEN', '')` | `config('services.owner_alert_bot.token', ...)` |
| `app/Services/OwnerAlertService.php` | 24 | `env('OWNER_TELEGRAM_ID', '0')` | `config('services.owner_alert_bot.owner_chat_id', ...)` |
| `app/Http/Controllers/OwnerBotController.php` | 19 | `env('OWNER_ALERT_BOT_TOKEN')` | `config('services.owner_alert_bot.token', ...)` |
| `app/Http/Controllers/OwnerBotController.php` | 192 | `env('TELEGRAM_CASHIER_BOT_TOKEN')` | `config('services.cashier_bot.token', ...)` — **wrong env var name** |
| `app/Http/Controllers/OwnerBotController.php` | 233 | `env('OWNER_TELEGRAM_ID', '38738713')` | `config('services.owner_alert_bot.owner_chat_id', ...)` |
| `app/Console/Commands/QueueHealthCheck.php` | 54 | `env('OWNER_ALERT_BOT_TOKEN')` | `config('services.owner_alert_bot.token', ...)` |
| `app/Console/Commands/QueueHealthCheck.php` | 55 | `env('OWNER_TELEGRAM_ID', '38738713')` | `config('services.owner_alert_bot.owner_chat_id', ...)` |
| `app/Console/Commands/TourSendReminders.php` | 23 | `env('OWNER_ALERT_BOT_TOKEN', '')` | `config('services.owner_alert_bot.token', ...)` |
| `app/Console/Commands/TourSendReminders.php` | 24 | `env('OWNER_TELEGRAM_ID', '0')` | `config('services.owner_alert_bot.owner_chat_id', ...)` |
| `app/Console/Commands/TourSendReminders.php` | 25 | `env('TELEGRAM_BOT_TOKEN_DRIVER_GUIDE', '')` | `config('services.driver_guide_bot.token', ...)` |
| `app/Observers/BookingObserver.php` | 22 | `env('TELEGRAM_BOT_TOKEN_DRIVER_GUIDE')` | `config('services.driver_guide_bot.token', ...)` |

**Total: 12 runtime env() calls across 6 files.**

---

## Addendum C: Queue Token Leak Inventory

### `SendTelegramNotificationJob` — TOKEN SERIALIZED TO QUEUE

| Property | Source |
|----------|--------|
| File | `app/Jobs/SendTelegramNotificationJob.php` |
| Class | `App\Jobs\SendTelegramNotificationJob` |
| Constructor | `public string $botToken` (line 21) — public property, serialized |
| Queue name | `telegram` (line 25) |
| Storage | Whatever `QUEUE_CONNECTION` is — `database` (jobs table) or `redis` |

**Dispatchers (callers that pass raw token into job):**

| Caller | File | Line | Which Bot Token |
|--------|------|------|-----------------|
| `OwnerAlertService::send()` | `app/Services/OwnerAlertService.php` | ~435-450 | `$this->botToken` (owner_alert) |

**Impact:** Plaintext bot token visible in:
- `jobs` table `payload` column (if using database queue driver)
- `failed_jobs` table `payload` column (if job fails)
- Redis keys (if using redis queue driver)
- Any queue monitoring dashboard (Horizon, etc.)

### `SendTelegramMessageJob` — SAFE

Reads `config('services.telegram_bot.token')` at execution time (line 36). Token not stored in job payload.

---

## Addendum D: Webhook Route Protection Inventory

| # | Route | Controller | Middleware | Secret Config | Header Validated | Verdict |
|---|-------|-----------|------------|---------------|-----------------|---------|
| 1 | `POST /api/telegram/webhook` | `TelegramController::handleWebhook` | None | None | No | **UNPROTECTED** |
| 2 | `POST /api/telegram/driver_guide_signup` | `TelegramDriverGuideSignUpController::handleWebhook` | None | `services.driver_guide_bot.webhook_secret` | No — secret stored but never checked | **UNPROTECTED** |
| 3 | `POST /api/telegram/bot/webhook` | `TelegramWebhookController::handle` | None | None | No | **UNPROTECTED** |
| 4 | `POST /api/booking/bot/webhook` | `BookingWebhookController::handle` | None | `services.telegram_booking_bot.secret_token` | No — configured but never checked | **UNPROTECTED** |
| 5 | `POST /api/telegram/pos/webhook` | `TelegramPosController::handleWebhook` | `ValidateTelegramRequest` | `services.telegram_pos_bot.secret_token` | **Commented out** (line 39) | **BROKEN** |
| 6 | `POST /api/telegram/cashier/webhook` | `CashierBotController::handleWebhook` | `verify.telegram.webhook` | `services.cashier_bot.webhook_secret` | Yes — `hash_equals()`, fail-closed | **PROTECTED** |
| 7 | `POST /api/telegram/owner/webhook` | `OwnerBotController::handleWebhook` | None | None | No | **UNPROTECTED** |
| 8 | `POST /api/telegram/housekeeping/webhook` | `HousekeepingBotController::handleWebhook` | None | None | No | **UNPROTECTED** |
| 9 | `POST /api/telegram/kitchen/webhook` | `KitchenBotController::handleWebhook` | None | None | No | **UNPROTECTED** |

**Summary: 1 protected, 1 broken, 7 unprotected.**

---

## Addendum E: Exact Ripgrep Commands Used

All commands run from repo root (`/home/oj/projects/jahongirnewapp`).

```bash
# A1: All env() calls referencing Telegram/bot tokens (in any PHP file)
rg 'env\(['\''"].*(?:TELEGRAM|BOT_TOKEN|OWNER_ALERT|CASHIER_BOT|HOUSEKEEPING|KITCHEN_BOT)' --glob '*.php' -n

# A2: All config() calls for telegram/bot service keys
rg 'config\(['\''"]services\.(telegram|owner_alert|driver_guide|cashier|housekeeping|kitchen|pos)' --glob '*.php' -n

# A3: All HTTP calls to Telegram API
rg 'api\.telegram\.org' --glob '*.php' -n

# A4: env() calls outside config/ and bootstrap/ (violation detection)
rg 'env\(' --glob '*.php' -t php -n | grep -v 'config/' | grep -v 'bootstrap/' | grep -vi 'vendor/' | grep -i 'telegram\|bot\|owner'

# A5: Variable assignments containing bot token
rg '\$.*[Bb]ot.*[Tt]oken|\$.*[Tt]oken.*[Bb]ot' --glob '*.php' -n

# A6: Webhook routes in api.php
rg 'telegram|webhook' routes/api.php -n -i

# A7: Token in job constructors (queue serialization check)
rg 'botToken|bot_token' app/Jobs/ -n

# A8: All Telegram-related middleware
rg 'class.*Telegram|class.*Webhook' app/Http/Middleware/ -n

# A9: Telegram-related model classes
rg 'class Telegram' app/Models/ -n

# A10: Secret/token validation in middleware
rg 'secret|Secret|hash_equals' app/Http/Middleware/ -n
```
